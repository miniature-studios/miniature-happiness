...
public class PersonalSpace : MonoBehaviour
{
    ...
    private float SlowDownFactorByDistance(EmployeeController employee)
    {
        ...
        return (radius - distance) / (radius - (2.0f * config.ActorRadius));
    }
    ...
    {
        if (controller.AverageVelocity.magnitude > config.MovelessVelocityThreshold)
        {
            return Vector3.zero;
        }
        ...
        foreach (EmployeeController employee in employeesInPersonalSpace)
        {
            ...
            float relative_velocity = Vector3.Dot(employee_to_this.normalized, employee_velocity);
