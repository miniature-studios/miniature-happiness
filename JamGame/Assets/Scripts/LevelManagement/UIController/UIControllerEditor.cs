...
{
    private string[] ActionNames => Assembly.GetAssembly(interfaceType).GetTypes()
               .Where(type => interfaceType.IsAssignableFrom(type) && !type.IsInterface)
                .Select(t => t.Name).ToArray();

    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        return (property.FindPropertyRelative("NamedAnimators").isExpanded, property.FindPropertyRelative("InterfaceMatcher").isExpanded) switch
        {
            ...
            (true, true) => namedAnimatorsListLength + interfaceMatcherListLength - EditorGUIUtility.singleLineHeight,
        };
    }
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        ReorderableList named_animators_list = GetNamedAnimatorsList(property, property.FindPropertyRelative("NamedAnimators"));
        namedAnimatorsListLength = named_animators_list.GetHeight();
        named_animators_list.DoList(position);

        ReorderableList interface_matcher_list = GetInterfaceMatcherList(property, property.FindPropertyRelative("InterfaceMatcher"));
        interfaceMatcherListLength = interface_matcher_list.GetHeight();
        interface_matcher_list.DoList(new Rect(position.x, position.y + named_animators_list.GetHeight(), position.width, position.height));
        ...
        {
            SerializedProperty array_value = property.FindPropertyRelative("NamedAnimators").GetArrayElementAtIndex(x);
            animatorsNames.Add(array_value.FindPropertyRelative("Name").stringValue);
        }

        List<InterfaceMatch> buffer_interface_matcher = new();

        SerializedProperty interface_matcher_array = property.FindPropertyRelative("InterfaceMatcher").Copy();
        for (int i = 0; i < interface_matcher_array.arraySize; i++)
        {
            buffer_interface_matcher.Add(new(interface_matcher_array.GetArrayElementAtIndex(i).FindPropertyRelative("InterfaceName").stringValue, new()));
            SerializedProperty _array = interface_matcher_array.GetArrayElementAtIndex(i).FindPropertyRelative("animatorBools");
            for (int j = 0; j < _array.arraySize; j++)
            {
                SerializedProperty _arr_element = _array.GetArrayElementAtIndex(j);
                buffer_interface_matcher.Last().animatorBools.Add(new(
        ...
        for (int i = 0; i < ActionNames.Count(); i++)
        {
            ...
            interface_matcher_array.GetArrayElementAtIndex(i).FindPropertyRelative("animatorBools").ClearArray();
            interface_matcher_array.GetArrayElementAtIndex(i).FindPropertyRelative("InterfaceName").stringValue = ActionNames[i];
            SerializedProperty animator_bools_array = interface_matcher_array.GetArrayElementAtIndex(i).FindPropertyRelative("animatorBools");

            for (int j = 0; j < animatorsNames.Count(); j++)
            {
                InterfaceMatch founded_interface_matcher = buffer_interface_matcher.Where(x => x.InterfaceName == ActionNames[i]).FirstOrDefault();
                float start_delay = 0;
                ...
                {
                    List<AnimatorBool> founded_animator_bool_array = founded_interface_matcher.animatorBools;
    ...
    private ReorderableList GetInterfaceMatcherList(SerializedProperty property, SerializedProperty listProperty)
    {
        ReorderableList list = new(property.serializedObject, listProperty, false, true, false, false)
        {
            drawHeaderCallback = rect =>
            {
                Rect newRect = new(rect.x + 10, rect.y, rect.width - 10, rect.height);
                listProperty.isExpanded = EditorGUI.Foldout(newRect, listProperty.isExpanded, listProperty.displayName + $" - {typeof(IDayAction).Name}" + $" - {listProperty.arraySize}", true);
            },
            ...
            drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                if (listProperty.isExpanded)
                {
                    ...
                    ReorderableList animator_bool_list = GetAnimatorBoolList(element, element.FindPropertyRelative("animatorBools"), element.FindPropertyRelative("InterfaceName").stringValue);
            ...
            elementHeightCallback = (int indexer) =>
            {
                ...
                return !listProperty.isExpanded ? 0 : GetAnimatorBoolList(element, element.FindPropertyRelative("animatorBools"), element.FindPropertyRelative("InterfaceName").stringValue).GetHeight() - EditorGUIUtility.singleLineHeight;
            }
        };
        return list;
    }

    private ReorderableList GetAnimatorBoolList(SerializedProperty property, SerializedProperty listProperty, string list_name)
    {
        ReorderableList list = new(property.serializedObject, listProperty, false, true, false, false)
        {
            ...
            {
                Rect newRect = new(rect.x + 10, rect.y, rect.width - 10, rect.height);
                listProperty.isExpanded = EditorGUI.Foldout(newRect, listProperty.isExpanded, list_name + $" - {listProperty.arraySize}", true);
            },
            ...
            drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                if (listProperty.isExpanded)
                {
                    ...
                    element.FindPropertyRelative("StartIsActive").boolValue = EditorGUI.Toggle(
                        new Rect(position.x, position.y, length, EditorGUIUtility.singleLineHeight),
                    ...
                    position.x += length;
                    length = 30;
                    element.FindPropertyRelative("StartDelay").floatValue = EditorGUI.FloatField(
                        new Rect(position.x, position.y, length, EditorGUIUtility.singleLineHeight),
        ...
        {
            return !listProperty.isExpanded ? 0 : list.elementHeight;
        };

        return list;
    }

    private ReorderableList GetNamedAnimatorsList(SerializedProperty property, SerializedProperty listProperty)
    {
        ReorderableList list = new(property.serializedObject, listProperty, false, true, true, true)
        {
            elementHeight = EditorGUIUtility.singleLineHeight,
            ...
            {
                Rect newRect = new(rect.x + 10, rect.y, rect.width - 10, rect.height);
                listProperty.isExpanded = EditorGUI.Foldout(newRect, listProperty.isExpanded, listProperty.displayName + $" - {listProperty.arraySize}", true);
            },
            ...
            drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                if (listProperty.isExpanded)
                {
                    ...
                    element.FindPropertyRelative("Animator").objectReferenceValue = EditorGUI.ObjectField(
                        new Rect(position.x, position.y, length, EditorGUIUtility.singleLineHeight),
                        element.FindPropertyRelative("Animator").objectReferenceValue, typeof(Animator), true
                    );
                }
            }
        };
